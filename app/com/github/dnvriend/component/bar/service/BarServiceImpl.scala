...
@Singleton
private[service] class BarServiceImpl @Inject() (@Named("bar-model") barActor: ActorRef, dao: BarDao, logger: Logger)(implicit system: ActorSystem, ec: ExecutionContext, mat: Materializer, timeout: Timeout) extends BarService {
  // poor man's typed actors
  override def process(cmd: BarCommand): Future[BarEvent] = for {
    now <- dao.now
    event <- (barActor ? MessageEnvelope(Random.nextInt(10).toString, cmd)).mapTo[BarEvent]
  } yield {
